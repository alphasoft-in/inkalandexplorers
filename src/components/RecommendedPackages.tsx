...
const RecommendedPackages: React.FC<Props> = ({ lang, excludeId, limit = 3 }) => {
  const recommended = useMemo(() => {
    // Filter out current package, filter only private packages
    const filtered = packages.filter(pkg => pkg.id !== excludeId && pkg.en.title.toLowerCase().includes('private'));
    // Use a deterministic offset based on excludeId to show different packages for different tours
    const offset = (excludeId || 0) % filtered.length;
    return [...filtered.slice(offset), ...filtered.slice(0, offset)].slice(0, limit);
  }, [excludeId, limit]);
              ...
              <div className="p-8 flex flex-col flex-grow">
                ...
                <div className="grid grid-cols-2 gap-4 mt-auto">
                  ...
                  <a 
                    ...
                    className="flex items-center justify-center bg-[#0b403a] text-white font-bold text-xs py-3 rounded-xl hover:bg-[#dfa126] hover:text-[#0b403a] transition-all duration-300 gap-2 shadow-lg shadow-[#0b403a]/20"
                  >
